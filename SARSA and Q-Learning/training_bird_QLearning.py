...
def evaluate_learning(sim, series_size=100, num_series=50, gamma=0.99, alpha=0.8, epsilon=0.001):
    ...
    for series in range(num_series):
        ...
        for episode in range(series_size):
            # training use Q policy
            Qvalue = Q_learning(sim, gamma=gamma, alpha=alpha, epsilon=epsilon, Qtable=Qvalue)
        ...
        np.save('Q_learning.npy', newQvalue)
...
plt.title('Q_learning Average score per series')
plt.tight_layout()
plt.show()
